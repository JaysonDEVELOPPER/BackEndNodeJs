const {
  ...
  updateProduit,
  deleteProduit,
} = require("../models/produits-models.js");
...
exports.getAll = async (req, res) => {
  try {
    ...
    res
      ...
      .json({
        data: getAllProduit,
        message: "Tous les produits bien récupérés",
      });
  } catch (error) {
    res.status(500).json({
      error: error,
      message: "Erreur 500 controllers produits getAll ",
    });
  }
};
...
exports.getProduitsId = async (req, res) => {
  try {
    ...
    if (!getById.length > 0) {
      return res
        .status(400)
        .json({ message: `Aucun produit est associer à ID:  ${id}` });
    }

    res.status(200).json({
      getById: getById,
      message: `ID : ${id} | correctement trouvé le produit`,
    });
  } catch (error) {
    res.status(500).json({
      error: error.message,
      message: "Erreur 500 controllers by id produits",
    });
  }
};
...
exports.getProduitsNom = async (req, res) => {
  try {
    ...
    if (!getNomProduit.length > 0) {
      return res
        .status(400)
        .json({ message: `Aucun produit est associer à ; ${nom}` });
    }
    res
      .status(200)
      .json({ getNomProduit, message: `Le produit ${nom} à été trouver` });
  } catch (error) {
    res
      .status(500)
      .json({ error: error, message: "Erreur 500 controller par nom produit" });
  }
};
...
exports.createProducts = async (req, res) => {
  try {
    ...
    res.status(200).json({
      createNewProduitId: createNewProduitId,
      message: `Le produit => ${createInfos.nom} a correctement été créé : `,
    });
  } catch (error) {
    res.status(500).json({
      error: error,
      message: "Erreur 500 controllers create produits",
    });
  }
};
...
exports.updateProduits = async (req, res) => {
  try {
    ...
    res.status(200).json({
      ...
      message: `Mise à jour du produit : ${produitUpdate.nom} ID: ${id}`,
    });
  } catch (error) {
    res
      .status(500)
      .json({ error: error, message: "Erreur controller update produits :" });
  }
};
...
exports.deleteProduits = async (req, res) => {
  try {
    ...
    if (!produitExist || produitExist.length === 0) {
      return res
        .status(404)
        .json({ message: `Produit avec l'ID ${id} n'existe pas.` });
    }
    ...
    res
      .status(500)
      .json({ error, message: "Erreur lors de la suppression du produit." });
  }
};
